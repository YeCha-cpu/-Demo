...
阶段 1 已实现 **L1 基础复制**（属性同步），现在就可以验证。战斗相关的复制要等阶段 2（L2）。

### 配置 PIE 双客户端

1. 点 **Play 按钮旁边的下拉箭头** → `Advanced Settings`
2. 设置：
   - `Number of Players` = **2**
   - `Net Mode` = **Play As Listen Server**
3. 点 Play，窗口布局选 `Two Players Side by Side`

> 💡 建议顺手在「编辑器偏好 → Level Editor → Play」里勾上 `Run Under One Process`——
> 两个客户端跑在同一个进程里，断点调试和日志查看都方便得多。

### 能验证什么

阶段 1 还没有伤害来源，所以这里验证的是**复制通道本身是通的**：

| 检查项 | 预期 |
|---|---|
| 两个窗口都能各自用 WASD 控制自己的角色 | ✅ 输入在本机处理，互不干扰 |
| 两边各自执行 `RPGPrintAttributes` | 数值一致（生命 100/100、攻击 10、防御 10） |
| Output Log 里搜 `GAS 初始化完成` | 应出现**两条**：一条标"运行在服务器"，一条标"运行在客户端" |

最后那条最能说明问题——**客户端那条日志的存在，证明"客户端不做授予能力"的分支走对了**。
...
能实时看到目标的 ASC 属性、激活中的 GE、拥有的标签。**联机调试 GAS 基本就靠它**，
比翻日志快一个数量级。
