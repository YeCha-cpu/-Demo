#  RPG —— UE5 动作角色扮演游戏（GAS 深度实践）项目架构设计文档

> UE 5.8.1· 联机 ARPG（Listen Server）· GAS 驱动 · 求职作品集
> 版本 v2.0 · 2026-09-12
...
> 注：Lyra 的 GameFeature 插件、以及社区主流的 GameplayAbilitySample 系列，都倾向功能文件夹。
...
> 💡 第 3 条特别重要，很多人会把"扣血"写在 AnimNotify 里——那样动画和数值就耦合死了，改数值要动动画资源，且无法单元测试。**Notify 只广播事件，GA 决定后果。**
...
> 💡 **关键理解**：Buff/Debuff **不需要在 Execution 里手动遍历**。因为 `Attack` / `Defense` 是 AttributeSet 上的属性，而 Buff 是通过 GE 的 Modifier 修改这些属性的——Execution 捕获到的是**已经被所有 GE 修改过后的当前值**。这就是"综合防御值、buff、debuff"的实现方式。讲清楚这点，说明你真的理解 GAS 的属性管线，而不是把它当黑盒。
...
> 💡 这是 GAS 的精髓：**用标签表达状态，用 OngoingTagRequirements 表达状态依赖**。如果你能对比"用 SetTimer 手写恢复逻辑"vs"用 OngoingTagRequirements"，并说出后者在联机下天然正确、在 GameplayDebugger 里可见、且策划可自行调整，这题就满分了。
...
> 💡 这是 AI + GAS 结合最经典的一个坑，高频问题："行为树怎么等待技能播放完毕？"
...
>
> 📖 **想快速 review 整个项目**：看 [`REVIEW_GUIDE.md`](./REVIEW_GUIDE.md) ——
> 19 次提交的变更编年史、审查锚点（哪些地方看着可疑但其实是对的）、
> 复发风险清单、以及**诚实标注的未验证项清单**。
...
| 29 | **`null == null` 会吃掉状态机的一个分支** | 用 `ASC == BoundASC.Get()` 同时表达"还没拿到"和"已经订上了" —— 而初始状态下两者**都是 null**，第一个分支成立，重试逻辑变成死代码。表现是"客户端看 AI 有血条、看玩家没有"，因为玩家 ASC 在 PlayerState 上、要等复制。**用一个条件表达两种状态前，先问"它们在初始值上会不会撞车"** |
| 30 | **换机制时要列出"旧机制顺带提供了什么保障"** | 把订阅重试从 `NativeTick` 换成 `TimerManager` 方向是对的（不该把订阅押在"隐藏控件还 tick 不 tick"这个 Slate 实现细节上），但**"每帧都跑"这个不起眼的保障被一起删掉了** —— 它恰好掩盖了上面那个判等顺序问题。新机制必须逐条补齐旧机制白送的东西 |
| 31 | **要"角色实体"就必须走 Avatar，不能走 Owner** | "玩家 ASC 放 PlayerState"这个决定的连带成本：`Cast<ARPG_BaseCharacter>(OwningActor)` 对玩家会**静默失败**（OwnerActor 是 PlayerState）。敌人碰巧能用。表现是"打敌人有飘字、打玩家没有"，且不报任何错。正解是 `Data.Target.GetAvatarActor()`（`Data.Target` 是 `UAbilitySystemComponent&`，不是 ActorInfo） |
| 32 | **"自己挨打本机隐藏"这类需求，判据点选在 Multicast 上** | 需求是"本地玩家受击时不显示飘字"，但**别人照样要看得见**。因为 Multicast 是在**受击者**身上发起的，在它的 `_Implementation` 开头判一次 `IsLocallyControlledPlayer()` 就够了 —— 不需要把"谁在看"这个信息一路传进 UI 层 |

> 💡 **用法建议**：不要一口气全讲。挑 2~3 个和岗位最相关的深入讲，其余作为"我还做了这些"的引子。**主动说出取舍和替代方案的缺点**，比只讲自己的实现更有说服力——那说明你是在做工程决策，而不是照抄教程。
